...
def adaboost_simple(X, Y, K, nSamples):
    """
    Adaboost with decision stump classifier as weak classifier
    :param X:   training examples (numSamples x numDim)
    :param Y:   training labels (numSamples x 1)
    :param K:   number of weak classifiers to select (scalar)
                (the _maximal_ iteration count - possibly abort earlier
                when error is zero)
    :param nSamples: number of training examples which are selected in
                each round (scalar). The sampling needs to be weighted!
    :return:
