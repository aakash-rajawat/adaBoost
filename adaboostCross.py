...
from eval_adaBoost_simpleClassifier import eval_adaBoost_simple_classifier

def adaboostCross(X, Y, K, nSamples, percent):
    """
    AdaBoost with an additional cross validation routine
    :param X:       training examples (numSamples x numDims )
    :param Y:       training lables (numSamples x 1)
    :param K:       number of weak classifiers to select (scalar)
    :param nSamples: number of training examples which are selected in each round. (scalar)
    ...
    for k in range(K): # Iterate over all classifiers
        ...
        classlabels, _ = eval_adaBoost_simple_classifier(testX, alphaK[:k + 1], para[:k + 1])
